...
Textile.prototype = {
  ...
  paragraphs: function(text) {
    ...
    // More?
    if (text.match(/<(blockquote|p|h[1-6]|pre|div|ul|ol|dir|menu|center|form|table|tr|td|tbody|th|thead|address|section|footer|title|meta|body|script|style|header|hgroup|br|dd|dl|dt|figure|fieldset|embed|col)>/i)) return text + '\n';
  ...
  inline: function(text) {
    ...
    text = text.replace(/!(([:alnum:]|\/|\?)[^!\s]+)!(?![^<]*<\/([a-zA-Z]))/g, '<img src="$1" alt="" />');
