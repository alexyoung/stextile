...
Textile.prototype = {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (!this.inPreformattedBlock) {
        ...
        if (this.inPreformattedContent) {
          line = this.indent(this.inPreformattedContent) + this.escapeHTML(line) + '\n';
        } else {
          line = line + '\n';
        }
        result += line || '';
  ...
  lastListType: function() {
    this.listTypes[this.listTypes.length - 1];
  },
  ...
  inline: function(text) {
    ...
    text = text.replace(/\*([^*]*)\*(?![^<]*<\/(code|pre))/g, '<strong>$1</strong>');
    text = text.replace(/_([^_]*)_(?![^<]*<\/(code|pre))/g, '<em>$1</em>');
    text = text.replace(/!([^_()]*)\((.*)\)!(?![^<]*<\/(code|pre))/g, '<img src="$1" alt="$2" />');
    text = text.replace(/!([^_]*)!(?![^<]*<\/(code|pre))/g, '<img src="$2" alt="" />');
  ...
  links: function(text) {
    text = text.replace(/"([^"]*)\s+\(([^)]*)\)":([^ ]*)/g, '<a title="$2" href="$3">$1</a>');
    text = text.replace(/"([^"]*)":([^ ]*)/g, '<a href="$2">$1</a>');

    // Move punctuation for bad link matches
    text = text.replace(/(\.|,)">([^<]*)<\/a>/g, '">$2</a>$1');
